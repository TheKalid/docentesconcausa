...
$archivo_json = __DIR__ . '/preguntas_ingreso_secundaria.json';

/************************************************************
 * CARGA OPTIMIZADA Y CACHÉ CORREGIDO
 ************************************************************/
function cargarBancoPreguntas($archivo_json) {
    // [CRÍTICO] Llave de caché única para SECUNDARIA
    $cache_key = 'banco_secundaria_ingreso';
...
if ($resultado_carga['status'] === 'success') {
    ...
    // [RENDIMIENTO] Eliminamos el shuffle() de PHP. Se hará en el cliente (JS).
} else {
...
<head>
    ...
    <title>Simulador Ingreso Secundaria | Vanguardia Magisterial</title>
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col">
    ...
    <nav class="bg-white shadow-md border-b border-gray-200 h-16 flex items-center justify-between px-4 lg:px-8 fixed w-full top-0 z-50">
        <div class="flex items-center gap-3">
            ...
            <span class="font-bold text-vm-dark hidden sm:block">Simulador <span class="text-vm-light">Ingreso Secundaria</span></span>
        </div>
