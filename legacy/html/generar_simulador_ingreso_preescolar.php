...
$archivo_json = __DIR__ . '/preguntas_ingreso_preescolar.json';

/************************************************************
 * CARGA OPTIMIZADA Y CACHÉ CORREGIDO
 ************************************************************/
function cargarBancoPreguntas($archivo_json) {
    // [CRÍTICO] La llave de caché DEBE ser única para cada simulador. 
    // Antes usabas 'banco_preguntas_vertical' lo que causaba colisiones con otros simuladores.
    $cache_key = 'banco_preescolar_ingreso';

    // 1. Intentar obtener desde APCu
...
if ($resultado_carga['status'] === 'success') {
    ...
    // [RENDIMIENTO] Eliminamos el shuffle() de PHP. 
    // Le pasamos la carga de trabajo al procesador del cliente (JS) para liberar el servidor.
} else {
...
<head>
    ...
    <title>Simulador | Vanguardia Magisterial</title>
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col">
    ...
    <nav class="bg-white shadow-md border-b border-gray-200 h-16 flex items-center justify-between px-4 lg:px-8 fixed w-full top-0 z-50">
        <div class="flex items-center gap-3">
            ...
            <span class="font-bold text-vm-dark hidden sm:block">Simulador <span class="text-vm-light">Ingreso Preescolar</span></span>
        </div>
    ...
    <script>
        ...
        function iniciar() {
            ...
            // [RENDIMIENTO] La mezcla se hace 100% en el cliente
            ...
            if (Array.isArray(p.opciones)) {
                p.opciones.forEach((opt, idx) => {
                    if (typeof opt === 'string') {
                        ...
                        opcionesParaRenderizar.push(opt);
                    }
                });
            } else if (typeof p.opciones === 'object') {
                for (const [key, value] of Object.entries(p.opciones)) {
