...
function activarSuscripcion($conexion, $userId, $plan_activo, $subscriptionId, $customerId, $fecha_proximo_pago) {
    ...
    $usos_protocolos = 0;
    $usos_bitacora = 0;
    $usos_examenes = 0; // 1. AÑADIMOS la nueva variable para exámenes

    if ($plan_activo == 1) { // Nivel 1 (Básico)
        $usos_plan_basico = 5;
        $usos_evaluacion = 5;
        $usos_fisica = 5;
    } elseif ($plan_activo == 2) { // Nivel 2 (Intermedio/Mentor)
        $usos_plan_intermedio = 7;
        $usos_evaluacion = 7;
        $usos_protocolos = 7;
        $usos_bitacora = 7;
        $usos_examenes = 5; // 2. ASIGNAMOS 5 créditos para exámenes al plan Mentor
    }
    // Puedes añadir una sección para el plan 3 (Líder) aquí si lo necesitas.

    // 3. ACTUALIZAMOS la consulta SQL para incluir la nueva columna
    ...
    // 4. AJUSTAMOS los parámetros del bind_param
    // Añadimos una 'i' más para el nuevo entero y la variable $usos_examenes al final
    $stmt->bind_param("issiiiiiiiii", 
        ...
        $usos_protocolos,
        $usos_bitacora,
        $usos_examenes, // Variable añadida
        $userId
    );
    $stmt->execute();
    $stmt->close();
