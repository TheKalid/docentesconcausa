...
// --- [NUEVO] Cabeceras de seguridad y Anti-Caché ---
header("X-Frame-Options: DENY");
header("X-XSS-Protection: 1; mode=block");
header("X-Content-Type-Options: nosniff");
header("Cache-Control: no-store, no-cache, must-revalidate, max-age=0");

// --- Bloque de seguridad y contador de usos ---
...
$_SESSION['usos_plan_intermedio'] = $usos_actuales; // Sincronizar sesión
$puede_generar = $usos_actuales > 0;

// --- [NUEVO] Cerramos BD y liberamos la pestaña inmediatamente ---
$stmt_usos->close();
$conexion->close();
session_write_close();
?>
...
<head>
    ...
    <title>Planeación Diferenciada (PAM) - Docentes con Causa</title>
    ...
    <style>
        ...
        .resultado-section h2 { margin-top: 0; color: var(--color-primario); }
        #btnCopiar { background-color: var(--color-exito); color: white; margin-top: 20px; border: none; font-weight: 600; cursor: pointer; transition: background-color 0.3s; padding: 10px 15px; border-radius: 8px; display:flex; align-items:center; justify-content:center; gap:8px;}
        /* ================= ESTILOS DEL BOTÓN PDF Y ENCABEZADO ================= */
        /* Contenedor flexible para alinear los botones de Copiar y PDF */
        .contenedor-botones { display: flex; flex-direction: column; gap: 10px; margin-top: 20px; }
        
        /* Diseño del botón rojo para exportar el PDF */
        ...
        /* Estilos del encabezado oficial exclusivo para el documento generado */
        ...
        .error-message { color:#dc3545; background-color:#f8d7da; border: 1px solid #f5c6cb; border-radius:8px; padding:15px; margin-top:15px;}
        .markdown-content h1, .markdown-content h2, .markdown-content h3 { color: var(--color-primario); border-bottom: 2px solid var(--color-acento); padding-bottom: 5px; margin-top: 25px; }
    </style>
</head>
<body>
    <div class="container">
        <header>
             <h1>🧮 Planeación Diferenciada Matemáticas (PAM)</h1>
        </header>
        ...
        <section class="form-section">
            <form id="planeacionForm" onsubmit="return false;">
                ...
                <h2>1. Datos Curriculares (Saberes y Pensamiento Científico)</h2>
                ...
                <div class="form-group">
                    <label for="ejeArticulador">Ejes Articuladores:</label>
                ...
                <h2>2. Composición del Grupo (Niveles Matemáticos)</h2>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="form-group">
                        <label for="ninos_presilabico">Nivel 1 - Básico (Conteo simple):</label>
                        <input type="number" id="ninos_presilabico" value="0" min="0" required>
                    </div>
                    <div class="form-group">
                        <label for="ninos_silabico">Nivel 2 - Intermedio (Suma y resta):</label>
                        <input type="number" id="ninos_silabico" value="0" min="0" required>
                    </div>
                    <div class="form-group">
                        <label for="ninos_silabico_alfabetico">Nivel 3 - Consolidado (Problemas simples):</label>
                        <input type="number" id="ninos_silabico_alfabetico" value="0" min="0" required>
                    </div>
                    <div class="form-group">
                        <label for="ninos_alfabetico">Nivel 4 - Avanzado (Algoritmos, complejidad):</label>
                ...
                <button type="button" id="btnGenerar" <?php if (!$puede_generar) echo 'disabled'; ?>>
                    <i class="fas fa-magic"></i> <?php echo $puede_generar ? 'Generar Planeación PAM' : '🚫 Usos Agotados'; ?>
                </button>
            </form>
        </section>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Autoguardado: Si hay datos en memoria, los colocamos en los inputs al cargar
        ...
        async function cargarDatosGrado() {
            ...
            try {
                // LLAMADA A LA CARPETA datos_plan_pam
                const response = await fetch(`datos_plan_pam/${grado}.json`); 
                ...
                alert("No se pudieron cargar los datos curriculares. Revisa que exista la carpeta 'datos_plan_pam' y los archivos JSON.");
            }
        }
        ...
        btnGenerar.addEventListener('click', async () => {
            ...
            if (!gradoTexto || !campoFormativoSelect.value || !contenidoSelect.value || !pdaSelect.value || ejesSeleccionados.length === 0) {
                 resultadoDiv.innerHTML = `<div class="error-message"><strong>Por favor, completa todos los campos curriculares, incluyendo los ejes articuladores.</strong></div>`;
                 return;
            }
            ...
            const datosParaEnviar = {
                ...
                composicionGrupo: {
                    nivel_inicial: parseInt(document.getElementById('ninos_presilabico').value, 10) || 0,
                    nivel_medio: parseInt(document.getElementById('ninos_silabico').value, 10) || 0,
                    nivel_avanzado: parseInt(document.getElementById('ninos_silabico_alfabetico').value, 10) || 0,
                    // Dejé las mismas variables exactas de tu código para evitar que algo se rompa
                    // nivel_avanzado extra:
                    alfabetico: parseInt(document.getElementById('ninos_alfabetico').value, 10) || 0 
                }
            };

            seccionResultado.style.display = 'block';
            resultadoDiv.innerHTML = `<div class="spinner"></div><p style="text-align:center; color:#1e3a8a;"><strong>Calculando algoritmos y ensamblando metodologías PAM...</strong><br><span style="font-size:0.9rem; color:#64748b;">Este es un proceso avanzado. Toma de 20 a 40 segundos.</span></p>`;
            contenedorBotones.style.display = 'none'; // Ocultamos el contenedor de botones
            
            // Guardamos los datos del docente en el navegador para la próxima vez
            ...
            try {
                // --- [NUEVO] Enviamos el Token Anti-Bots a PHP ---
                const response = await fetch('procesar_planeacion_avanzada_pam.php', {
                    method: 'POST',
                    headers: { 
                ...
                if (data.status === 'completo') {
                    ...
                    // Obtenemos los valores de los inputs o usamos textos por defecto
                    const nomDoc = inputNombreDocente.value || 'Docente PAM';
                    const nomEsc = inputNombreEscuela.value || 'Institución Educativa';
                    
                    // Inyectamos el encabezado oficial al principio del HTML resultante
                    let htmlResultado = `
                    ...
                    if (planData.sugerencias_didacticas && planData.sugerencias_didacticas.length > 0) {
                        htmlResultado += '<h3>💡 Sugerencias Didácticas PAM</h3><ul>';
                        planData.sugerencias_didacticas.forEach(s => htmlResultado += `<li>${s}</li>`);
                        htmlResultado += '</ul>';
                    }

                    resultadoDiv.innerHTML = htmlResultado; 
                    // Mostramos el contenedor con los dos botones
                    contenedorBotones.style.display = 'flex';
                }

            } catch (error) {
                 console.error(error);
                 resultadoDiv.innerHTML = `<div class="error-message"><strong>Ocurrió un error:</strong><br>${error.message}<br><br><span style="color:#333; font-weight:normal;">No se descontó tu crédito.</span></div>`;
            } finally {
                if (parseInt(contadorDisplay.innerText, 10) > 0) {
                    btnGenerar.disabled = false;
                    btnGenerar.innerHTML = '<i class="fas fa-magic"></i> Generar Planeación PAM';
                } else {
                    btnGenerar.disabled = true;
