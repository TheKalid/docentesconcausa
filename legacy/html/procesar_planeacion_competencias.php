<?php
// procesar_planeacion_competencias.php (VERSIÓN ENDURECIDA, AUDITADA Y DE ALTA CONCURRENCIA)
...
// ==========================================
// [SEGURIDAD] CABECERAS DE PROTECCIÓN HTTP
// ==========================================
...
try {
    ...
    $asignatura = !empty($data['asignatura1']) ? trim(strip_tags($data['asignatura1'])) : '';

    // ==========================================
    // CEREBRO DE LA IA: MODELO POR COMPETENCIAS
    // ==========================================
    $PROMPT_SISTEMA = <<<EOT
Eres un experto en diseño curricular tradicional y constructivista. Tu tarea es diseñar una secuencia didáctica que, aunque tome como punto de partida un PDA de la Nueva Escuela Mexicana (NEM), se estructure ESTRICTAMENTE bajo el modelo de Competencias (Habilidades, Actitudes y Valores).

PRIORIDAD EDUCATIVA:
Prioriza actividades de desarrollo cognitivo duro (ejercicios prácticos de lectoescritura, razonamiento lógico-matemático, análisis de textos o resolución de problemas).

FORMATO DE SALIDA (JSON OBLIGATORIO):
{
  "datos_principales": {
    "tema_central": "Nombre del tema o lección",
    "pda_ancla": "El PDA base utilizado",
    "competencias": "Describe las Competencias, Habilidades, Actitudes y Valores a desarrollar"
  },
  "lista_materiales": ["Material 1", "Material 2"],
  "secuencia_completa": "Todo el contenido detallado de las sesiones en formato Markdown.",
  "evaluacion_formativa": ["Rúbrica para evaluar la competencia", "Criterio de evaluación 2"],
...
## Sesión 1
**Inicio (Introducción):** - Actividades para rescatar saberes previos.
- Enganche
- Explicacion

**Desarrollo:** - Instrucción directa del docente.
- Práctica guiada e independiente. (Fomento de la competencia).

**Cierre:** - Reflexión sobre lo aprendido.
- Revisión de ejercicios.
- Tarea o consolidación.

Reglas: No uses jerga exclusiva de la NEM como "Proyectos Comunitarios" a menos que se integren lógicamente. Mantén el enfoque en la competencia académica individual.
EOT;

    $userMessageContent = "Grado General: $grado\nDuración: $tiempo\n\n";
    if ($asignatura) {
        $userMessageContent .= "Asignatura: $asignatura\n";
    }
    $userMessageContent .= "Campo Formativo: $campo1\nContenido: $cont1\nPDA Principal: $pda1\n";
    ...
    $herramienta_usada = "Planeación por Competencias"; // Match exacto con Analytics
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        throw new Exception("Error estructurando la secuencia didáctica.");
    }
    ...
    if ($e->getMessage() !== 'Has agotado tus créditos para este plan.') {
        try {
            ...
            $stmt_del_log = $conexion->prepare("DELETE FROM historial_uso WHERE usuario_id = ? AND herramienta = 'Planeación por Competencias' ORDER BY id DESC LIMIT 1");
            ...
            error_log("CRÍTICO: No se pudo reembolsar el crédito de competencias al usuario ID: " . $userId);
        }
    }
