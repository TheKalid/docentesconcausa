...
// Generar Token CSRF si no existe (Firma Anti-Bots)
...
<head>
    ...
    <title>Planeación por Contenidos - Planeando con Causa</title>
...
<body>
    <div class="container">
        <header>
            <h1>🎯 Planeación por Asignatura (No proyectos)</h1>
            <p>Estructura secuencias didácticas priorizando Habilidades, Actitudes y Valores cognitivos.</p>
        ...
        <section class="form-section">
            <form id="planeacionForm">
                ...
                <div class="eje-container">
                    ...
                    <label for="campoFormativo1">Campo Formativo:</label>
                    ...
                    <label for="pda1">PDA Base:</label>
                    <select id="pda1" required></select>
                </div>

                <label for="tiempo">Duración de la secuencia:</label>
                ...
                <button type="button" id="btnGenerar"><i class="fas fa-brain"></i> Construir Secuencia por Competencias</button>
            </form>
        </section>

        <section class="resultado-section" style="display:none;" id="seccionResultadoContenedor">
            <h2>📝 Tu Planeación por Competencias</h2>
    ...
    <script>
        ...
        async function actualizarOpciones() {
            ...
            document.getElementById(`campoFormativo1`).innerHTML = '<option value="">-- Selecciona --</option>';
            document.getElementById(`contenido1`).innerHTML = '<option value="">-- Selecciona --</option>';
            document.getElementById(`pda1`).innerHTML = '<option value="">-- Selecciona --</option>';
            if(isSecundaria) document.getElementById(`asignatura1`).innerHTML = '<option value="">-- Selecciona --</option>';
            ...
            try {
                ...
                if (isSecundaria) {
                    ...
                    const asigSelect = document.getElementById(`asignatura1`);
        ...
        btnGenerar.addEventListener('click', async () => {
            const payload = {
                ...
                asignatura1: document.getElementById('asignatura1') ? document.getElementById('asignatura1').value : ''
            };
            
            if (!payload.grado || !payload.campo1 || !payload.cont1 || !payload.pda1 || !payload.tiempo) {
                alert('Completa todos los datos curriculares requeridos.'); return;
            }

            formSection.style.display = 'none'; 
            seccionResultadoContenedor.style.display = 'block'; 
            resultadoDiv.innerHTML = '<div class="spinner"></div><p style="text-align: center; color: #1e3a8a;">Estructurando competencias y habilidades cognitivas...<br><span style="font-size:0.9rem; color:#64748b;">Esto puede tardar entre 20 y 30 segundos.</span></p>';

            try {
                const response = await fetch('procesar_planeacion_competencias.php', {
                    method: 'POST',
                    headers: { 
                    ...
                    let html = `
                        <div id="area-impresion-pdf">
                            <div class="markdown-content">
                                ...
                                <p><strong>PDA Base:</strong> ${escapeHTML(plan.datos_principales.pda_ancla)}</p>
                                ...
                                <h4><i class="fas fa-chalkboard-teacher"></i> Secuencia Didáctica por Competencias</h4>
                                ${marked.parse(plan.secuencia_completa)}
                                
                                <h4><i class="fas fa-clipboard-check"></i> Evaluación Formativa Basada en Desempeño</h4>
        ...
        resultadoDiv.addEventListener('click', function(e) {
            ...
            if (pdfBtn) {
                ...
                // [SEGURIDAD] Configuración optimizada para PDF
                html2pdf().set({
                    margin: [15, 15, 15, 15], 
                    filename: 'Secuencia_Competencias.pdf',
