<?php
// =======================================================================
// 1. SEGURIDAD Y RENDIMIENTO (BLINDAJE)
// OJO PARA HBAILITAR EL BOTON <button type="submit" class="btn-submit">Subir Recurso</button>
// =======================================================================
header("X-Frame-Options: DENY");
header("X-XSS-Protection: 1; mode=block");
header("X-Content-Type-Options: nosniff");

// [SEGURIDAD CRÍTICA] Apagar errores en producción para no revelar rutas del servidor
ini_set('display_errors', 0);
ini_set('log_errors', 1);

session_start();

// Generar Token CSRF para el formulario de subida
if (empty($_SESSION['csrf_token'])) {
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
}
$csrf_token = $_SESSION['csrf_token'];

// [RENDIMIENTO] Liberar sesión inmediatamente
session_write_close();

require_once 'config.php';

// =======================================================================
// 2. SISTEMA DE CACHÉ PARA LA BASE DE DATOS (Offloading MySQL)
// =======================================================================
$recursos = [];
$error_message = null;
$cache_key = 'biblioteca_recursos_publicos';

try {
    // Intentar leer desde la memoria RAM primero
    if (function_exists('apcu_fetch')) {
        $recursos = apcu_fetch($cache_key);
    }
    
    // Si no está en RAM (o expiró), consultamos a MySQL
    if (empty($recursos)) {
        $stmt = $pdo->query("SELECT * FROM recursos ORDER BY fecha_subida DESC");
        $recursos = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Guardar en RAM por 5 minutos (300 segundos)
        if (function_exists('apcu_store')) {
            apcu_store($cache_key, $recursos, 300);
        }
    }
} catch (PDOException $e) {
    $error_message = "Error al obtener los recursos de la biblioteca.";
    error_log("Error en consulta de biblioteca.php: " . $e->getMessage()); 
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        .return-section{text-align:center; margin-top:50px}
        ...
        <main class="main-layout">
            ...
            <aside class="upload-section">
                ...
                <form action="subir_recurso.php" method="POST" enctype="multipart/form-data">
                    ...
                    <button type="button" class="btn-submit" disabled style="background-color: #94a3b8; cursor: not-allowed;" title="La subida de recursos está en mantenimiento">Subida deshabilitada temporalmente</button>
                </form>
            </aside>
